...
if k == 1
	...
	for n = 1:85 % no. of years minus 1 incomplete winter season
	 ...
	 iceprod_20C(n,k) = sum(WI2O(ffrz).*rep_cellarea(ffrz))*30;
	 % total positive area diverged each winter in m^2
	 pos_areadiv_20C(n,k) = sum((Wdiv(fd)*(30/100)).*rep_cellarea(fd)); % factor (30/100) converts %/day to /month
	 % net area diverged each winter in m^2
	 net_areadiv_20C(n,k) = sum((Wdiv(fnd)*(30/100)).*rep_cellarea(fnd)); % factor (30/100) converts %/day to /month
         % snow - count over >15% ice cells
         hs_20C(n,k) = sum((Wsnow(fp).*rep_cellarea(fp))/sum(rep_cellarea(fp)));
	 % September sea ice area
	 SepSIA_20C(n,k) = squeeze(nansum(nansum(SepSIC.*cellarea)));
         % temperature - count over all ocean cells
	 SAT_20C(n,k) = sum((WSAT(fn).*rep_cellarea(fn))/sum(rep_cellarea(fn)));
	 % freezing area days
...
if k >= 34
        ...
        SAT = ncread(strcat(fnames_RCP85{k},'.cice.h.Tair_nh.200601-210012.nc'),'Tair',[1,1,1],[291,48,900]); % saved as 2006-2100 
...
% first of all, process the first year - note that this will be associated with 20C as the last year.
    % take winter months
	 ...
	 fpd = intersect(fd,fp); % find cells where >15% ice and divergent
        ...
        % total ice production (m^3). Need factor 30 to convert from mean daily to mean monthly. 
	 iceprod_RCP85(n,k) = sum(WI2O(ffrz).*rep_cellarea(ffrz))*30;
	 % total positive area diverged each winter in m^2
	 pos_areadiv_RCP85(n,k) = sum((Wdiv(fd)*(30/100)).*rep_cellarea(fd)); % factor (30/100) converts %/day to /month
	 % net area diverged each winter in m^2
	 net_areadiv_RCP85(n,k) = sum((Wdiv(fnd)*(30/100)).*rep_cellarea(fnd)); % factor (30/100) converts %/day to /month
         % snow - count over >15% ice cells
         hs_RCP85(n,k) = sum((Wsnow(fp).*rep_cellarea(fp))/sum(rep_cellarea(fp)));
	 % September sea ice area
	 SepSIA_RCP85(n,k) = squeeze(nansum(nansum(SepSIC.*cellarea)));
         % temperature - count over cells where ice present at >15%
         SAT_RCP85(n,k) = sum((WSAT(fn).*rep_cellarea(fn))/sum(rep_cellarea(fn)));
         % freezing area days
         M1 = ones(size(WI2O))*30;
...
save /home/ocean_personal_data/samc/CESM/variables/CESM_iceprod_vars_35.mat GR_20C GR_RCP85 iceprod_20C iceprod_RCP85 net_areadiv_20C net_areadiv_RCP85 pos_areadiv_20C pos_areadiv_RCP85 hs_20C hs_RCP85 SepSIA_20C SepSIA_RCP85 SAT_20C SAT_RCP85 frz_area_days_20C frz_area_days_RCP85 Fw_20C Fw_RCP85
