	...
	for n = 1:85 % no. of years minus 1 incomplete winter season
	 ...
	 fpf = intersect(fp,ffrz); % points where ice >15% and freezing
        % area-weighted growth rate only over cells where ice is growing
         GR_20C(n,k-35) = sum((WI2O(ffrz).*rep_cellareaxx(ffrz))/sum(rep_cellareaxx(ffrz)));
         % net div - count and normalise over freezing and >15% ice cells
         netdiv_20C(n,k-35) = sum((Wdiv(fp).*rep_cellareaxx(fp))/sum(rep_cellareaxx(fp)));
         % pos div - count over freezing and divergent points, normalise over all freezing
         posdiv_20C(n,k-35) = sum((Wdiv(fd).*rep_cellareaxx(fd))/sum(rep_cellareaxx(fp)));
         % snow - count over >15% ice cells
         hs_20C(n,k-35) = sum((Wsnow(fp).*rep_cellareaxx(fp))/sum(rep_cellareaxx(fp)));
         % temperature - count over cells where ice present at >15%
         SAT_20C(n,k-35) = sum((WSAT(fp).*rep_cellareaxx(fp))/sum(rep_cellareaxx(fp)));
	 % freezing area days
	 M1 = ones(size(WI2O))*30;
	 frz_area_days_20C(n,k-35) = sum(M1(ffrz).*rep_cellareaxx(ffrz));
	 % heat fluxes - count and normalise over days where ice present at >15%
	 Fw_20C(n,k-35) = sum((WFw(ffrz).*rep_cellareaxx(ffrz))/sum(rep_cellareaxx(ffrz)));
	 end
    ...
    % take winter months
	 ...
	 fp = find(WSIC>0.15); % find cells with more than 15% sea ice
        ...
        for n = 1:73 % no. of years mins 2 incomplete winter seasons
         ...
         fpf = intersect(fp,ffrz); % points where ice >15% and freezing
        % area-weighted growth rate only over cells where ice is growing
         GR_RCP85(n,k-35) = sum((WI2O(ffrz).*rep_cellareaxx(ffrz))/sum(rep_cellareaxx(ffrz)));
         % net div - count and normalise over freezing and >15% ice cells
         netdiv_RCP85(n,k-35) = sum((Wdiv(fp).*rep_cellareaxx(fp))/sum(rep_cellareaxx(fp)));
         % pos div - count over freezing and divergent points, normalise over all freezing
         posdiv_RCP85(n,k-35) = sum((Wdiv(fd).*rep_cellareaxx(fd))/sum(rep_cellareaxx(fp)));
         % snow - count over >15% ice cells
         hs_RCP85(n,k-35) = sum((Wsnow(fp).*rep_cellareaxx(fp))/sum(rep_cellareaxx(fp)));
         % temperature - count over cells where ice present at >15%
         SAT_RCP85(n,k-35) = sum((WSAT(fp).*rep_cellareaxx(fp))/sum(rep_cellareaxx(fp)));
         % freezing area days
         M1 = ones(size(WI2O))*30;
         frz_area_days_RCP85(n,k-35) = sum(M1(ffrz).*rep_cellareaxx(ffrz));
	 % heat fluxes - count and normalise over days where ice present at >15%
	 Fw_RCP85(n,k-35) = sum((WFw(ffrz).*rep_cellareaxx(ffrz))/sum(rep_cellareaxx(ffrz)));
	 end     
 fprintf('finished RCP85 ensemble # %d\n',k)
end

save /home/ocean_personal_data/samc/CESM/variables/CESM_growth_rate_params_frz_35.mat GR_20C GR_RCP85 netdiv_20C netdiv_RCP85 posdiv_20C posdiv_RCP85 hs_20C hs_RCP85 SAT_20C SAT_RCP85 frz_area_days_20C frz_area_days_RCP85 Fw_20C Fw_RCP85
